...
class SalesReservationService
{
    ...
    public function createReservation(array $data, User $user): SalesReservation
    {
        ...
        try {
            ...
            $unit = ContractUnit::with(['secondPartyData:id,contract_id'])
                ->where('id', $data['contract_unit_id'])
                ->lockForUpdate()
                ->first();
            ...
            if ((int) $unit->secondPartyData?->contract_id !== (int) $data['contract_id']) {
                throw new \InvalidArgumentException('Selected unit does not belong to the selected contract');
            }
            ...
            // Preliminary (حجز مبدئي) uses defaults for optional fields
            $isPreliminary = ($data['reservation_type'] ?? '') === 'preliminary';
            if ($isPreliminary) {
                $data['client_nationality'] = $data['client_nationality'] ?? 'غير محدد';
                $data['client_iban'] = $data['client_iban'] ?? '';
                $data['payment_method'] = $data['payment_method'] ?? 'cash';
                $data['down_payment_status'] = $data['down_payment_status'] ?? 'refundable';
                $data['purchase_mechanism'] = $data['purchase_mechanism'] ?? 'cash';
            }

            // Determine initial status based on reservation type
            $status = $data['reservation_type'] === 'negotiation'
                ? ReservationStatus::UNDER_NEGOTIATION
    ...
    /**
     * Get a single reservation by id (with view access check).
     */
    public function getReservation(int $id, User $user): SalesReservation
    {
        $reservation = SalesReservation::with(['contract', 'contractUnit', 'marketingEmployee'])
            ->findOrFail($id);

        // Same visibility as list: owner or admin
        if ($reservation->marketing_employee_id !== $user->id && !$user->hasRole('admin')) {
            throw new \Illuminate\Auth\Access\AuthorizationException('Unauthorized to view this reservation');
        }
    ...
    public function getReservations(array $filters, User $user): LengthAwarePaginator
    {
        ...
        // Include cancelled or not (when explicitly requesting status=cancelled, include them)
        $statusFilter = $filters['status'] ?? null;
        $wantCancelledOnly = $statusFilter === ReservationStatus::CANCELLED;
        if (!$wantCancelledOnly && (empty($filters['include_cancelled']) || $filters['include_cancelled'] === 'false' || $filters['include_cancelled'] === 0)) {

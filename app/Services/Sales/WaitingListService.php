...
class WaitingListService
{
    ...
    /**
     * Get a single waiting list entry by id (owner, leader, or admin).
     */
    public function getWaitingListEntry(int $id, User $user): SalesWaitingList
    {
        $entry = SalesWaitingList::with(['contract', 'contractUnit', 'salesStaff'])->findOrFail($id);

        if ($entry->sales_staff_id !== $user->id && !$user->isSalesLeader() && !$user->hasRole('admin')) {
            throw new \Illuminate\Auth\Access\AuthorizationException('Unauthorized to view this waiting list entry');
        }
    ...
    {
        $query = SalesWaitingList::with(['contract.info', 'contractUnit', 'salesStaff', 'convertedToReservation', 'convertedBy']);
    ...
    public function convertToReservation(int $waitingListId, array $reservationData, User $leader): SalesReservation
    {
        ...
        try {
            ...
            $unit = ContractUnit::with(['secondPartyData:id,contract_id'])->findOrFail($waitingEntry->contract_unit_id);
            $resolvedContractId = (int) ($unit->secondPartyData?->contract_id ?? $waitingEntry->contract_id);

...
class DepositService
{
    ...
    /**
     * Get deposits for management view. When $user is provided and not admin/accountant, scope to own or team.
     */
    public function getDepositsForManagement(?string $status = null, ?string $from = null, ?string $to = null, int $perPage = 15, ?User $user = null)
    {
        $query = Deposit::with([
    ...
    public function getDepositStatsByProject(int $contractId): array
    {
        ...
        $received = $deposits->whereIn('status', DepositStatus::receivedOrConfirmed());
     ...
     */
    /**
     * Get deposit details. When $user is provided and not admin/accountant, ensure deposit is visible to user.
     */
    public function getDepositDetails(int $depositId, ?User $user = null): array
    {
        ...
        if ($user && !$user->hasRole('admin') && !$user->hasRole('accountant')) {
            $allowedUserIds = $user->team
                ? User::where('team', $user->team)->pluck('id')
                : collect([$user->id]);
            $marketerId = $deposit->salesReservation?->marketing_employee_id;
            if ($marketerId === null || !$allowedUserIds->contains($marketerId)) {
                throw new \Illuminate\Auth\Access\AuthorizationException('Unauthorized to view this deposit');
            }
        }

...
class ContractUnitService
{
    ...
    public function uploadCsvByContractId(int $contractId, UploadedFile $file, string $version = 'v1'): array
    {
        DB::beginTransaction();
        try {
            ...
            // Process CSV (version v2 supports bedrooms, floor columns)
            $unitsCreated = $this->processCsvFile($file, $secondPartyData->id, $version);
     ...
     * Version v2 adds bedrooms and floor column mapping.
     */
    private function processCsvFile(UploadedFile $file, int $secondPartyDataId, string $version = 'v1'): int
    {
        ...
        // Map CSV columns to database fields (v2 includes bedrooms, floor)
        $columnMap = [
            'unit_type' => ['unit_type', 'type', 'نوع_الوحدة', 'نوع'],
            'unit_number' => ['unit_number', 'number', 'رقم_الوحدة', 'رقم', 'unit_code', 'كود_الوحدة'],
        ...
        if ($version === 'v2') {
            $columnMap['bedrooms'] = ['bedrooms', 'bedrooms_count', 'غرف', 'عدد_الغرف'];
            $columnMap['floor'] = ['floor', 'floor_number', 'الدور', 'الطابق'];
        }

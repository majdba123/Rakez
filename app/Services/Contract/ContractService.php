...
class ContractService
{
    ...
    public function getContracts(array $filters = [], int $perPage = 15): LengthAwarePaginator
    {
        try {
            ...
            if (isset($filters['project_name']) && !empty($filters['project_name'])) {
                $query->where('project_name', 'like', SearchTerm::contains($filters['project_name']));
            }
    ...
    public function getContractLocationsForAdmin(array $filters = [], int $perPage = 200): LengthAwarePaginator
    {
        try {
            ...
            // Filter by project name (search)
            if (isset($filters['project_name']) && !empty($filters['project_name'])) {
                $query->where('contracts.project_name', 'like', SearchTerm::contains($filters['project_name']));
            }
    ...
    public function storeContractInfo(int $contractId, array $data, ?Contract $contract = null): ContractInfo
    {
        ...
        try {
            ...
            $fixed = [
                ...
                'first_party_name' => config('contract.first_party_name'),
                'first_party_cr_number' => config('contract.first_party_cr_number'),
                'first_party_signatory' => config('contract.first_party_signatory'),
                'first_party_phone' => config('contract.first_party_phone'),
                'first_party_email' => config('contract.first_party_email'),
            ];

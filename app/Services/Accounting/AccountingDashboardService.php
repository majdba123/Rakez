...
class AccountingDashboardService
{
    ...
    {
        $query = SalesReservation::where('status', ReservationStatus::CONFIRMED);
    ...
    {
        $query = Deposit::whereIn('status', DepositStatus::receivedOrConfirmed());
    ...
    {
        $query = Deposit::where('status', DepositStatus::REFUNDED);

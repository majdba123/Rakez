...
class AccessExplanationEngine
{
    ...
    /**
     * Explain access for a specific route and optional entity (v2 API).
     * Returns: allowed, missing_permissions, human_reason, suggested_routes (from config only, filtered by user).
     *
     * @return array{allowed: bool, missing_permissions: array, human_reason: string, suggested_routes: array}
     */
    public function explainAccess(User $user, string $route, ?string $entityType = null, ?int $entityId = null): array
    {
        $missingPermissions = [];
        $allowed = true;
        $humanReason = '';
        ...
        if ($entityType !== null && $entityId !== null) {
            ...
            if ($modelClass && class_exists($modelClass)) {
                $instance = $modelClass::find($entityId);
                if (! $instance) {
                    $allowed = false;
                    $humanReason = 'The requested resource was not found.';
                } elseif (! Gate::forUser($user)->allows('view', $instance)) {
                    $allowed = false;
                    $humanReason = 'You do not have permission to view this resource.';
                    if (! $this->hasTypeLevelAccess($user, $modelClass)) {
                        $missingPermissions = $this->getPermissionsForModel($modelClass);
                    }
                }
            }
        }

        $suggestedRoutes = $this->getSuggestedRoutesForUser($user);

        if ($allowed && $humanReason === '') {
            $humanReason = 'You have access to this resource.';
        }
    ...
    private function getPermissionsForModel(string $modelClass): array
    {
        $map = [
            ...
            \App\Models\Lead::class => ['marketing.projects.view'],
        ];
    ...
    /**
     * Suggested routes from config/ui_routes.php that the user can access.
     *
     * @return array<int, array{route: string, label: string}>
     */
    private function getSuggestedRoutesForUser(User $user): array
    {
        $routes = config('ui_routes', []);
        $suggested = [];
        foreach ($routes as $routeKey => $config) {
            ...
            if (is_array($permission)) {
                ...
                if ($can) {
                    $suggested[] = ['route' => $routeKey, 'label' => $config['label'] ?? $routeKey];
                }
            } elseif (is_string($permission) && $user->can($permission)) {
                $suggested[] = ['route' => $routeKey, 'label' => $config['label'] ?? $routeKey];
            }
        }

...
class StoreReservationRequest extends FormRequest
{
    ...
    {
        if ($this->input('reservation_type') === 'preliminary') {
            if (empty($this->contract_date)) {
                $this->merge(['contract_date' => now()->format('Y-m-d')]);
            }
            if ($this->filled('notes') && ! $this->filled('negotiation_notes')) {
                $this->merge(['negotiation_notes' => $this->input('notes')]);
            }
        }
    }
    ...
    {
        $isPreliminary = $this->input('reservation_type') === 'preliminary';

        return [
            'contract_id' => 'required|exists:contracts,id',
            'contract_unit_id' => 'required|exists:contract_units,id',
            'contract_date' => $isPreliminary ? 'nullable|date' : 'required|date',
            'reservation_type' => 'required|in:confirmed_reservation,negotiation,preliminary',
            ...
            'client_nationality' => $isPreliminary ? 'nullable|string|max:100' : 'required|string|max:100',
            'client_iban' => $isPreliminary ? 'nullable|string|max:100' : 'required|string|max:100',
            'payment_method' => $isPreliminary ? 'nullable|in:bank_transfer,cash,bank_financing' : 'required|in:bank_transfer,cash,bank_financing',
            'down_payment_amount' => 'required|numeric|min:0',
            'down_payment_status' => $isPreliminary ? 'nullable|in:refundable,non_refundable' : 'required|in:refundable,non_refundable',
            'purchase_mechanism' => $isPreliminary ? 'nullable|in:cash,supported_bank,unsupported_bank' : 'required|in:cash,supported_bank,unsupported_bank',

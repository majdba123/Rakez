...
class SalesReservationController extends Controller
{
    ...
    {
        try {
            $reservation = $this->reservationService->getReservation($id, request()->user());
    ...
    /**
     * Download reservation voucher (receipt). Regenerates PDF if missing or file deleted.
     */
    public function downloadVoucher(int $id): Response|JsonResponse
    {
        try {
            $reservation = SalesReservation::findOrFail($id);

            $user = request()->user();
            $isOwner = $reservation->marketing_employee_id === $user->id;
            $canDownload = $isOwner || $user->hasRole('admin') || $user->hasRole('sales_leader');
            if (!$canDownload) {
                return response()->json([
                    'success' => false,
            ...
            $path = $reservation->voucher_pdf_path;
            if (!$path) {
                return response()->json([
                    'success' => false,
            ...
            if (!Storage::disk('public')->exists($path)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Voucher file is missing',
                ], 404);
            }

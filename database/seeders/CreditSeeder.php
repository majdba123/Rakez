...
class CreditSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $total = count($bankIds);
        // Allocate with minimums so all Credit Bookings tabs have data (Sold, in_progress, rejected, etc.)
        $soldCount = min($total, max(20, (int) round($total * 0.15)));
        $remaining = $total - $soldCount;
        $rejectedCount = min($remaining, max(15, (int) round($total * 0.10)));
        $remaining -= $rejectedCount;
        $titleTransferCount = min($remaining, max(10, (int) round($total * 0.12)));
        $remaining -= $titleTransferCount;
        $inProgressCount = min($remaining, max(15, (int) round($total * 0.30)));
        $pendingCount = $remaining - $inProgressCount;

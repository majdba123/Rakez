...
class SalesReservationTest extends TestCase
{
    ...
    public function test_create_reservation_rejects_unit_from_different_contract()
    {
        $otherContract = Contract::factory()->create(['status' => 'ready']);
        $otherSecondPartyData = SecondPartyData::factory()->create(['contract_id' => $otherContract->id]);
        $otherUnit = ContractUnit::factory()->create([
            'second_party_data_id' => $otherSecondPartyData->id,
            'status' => 'available',
            'price' => 600000,
        ]);

        $data = $this->getValidReservationData();
        $data['contract_unit_id'] = $otherUnit->id;

        $response = $this->actingAs($this->salesUser, 'sanctum')
            ->postJson('/api/sales/reservations', $data);

        $response->assertStatus(422);
        $response->assertJsonValidationErrors(['contract_unit_id']);
    }

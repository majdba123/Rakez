...
class PaymentPlanTest extends TestCase
{
    ...
    public function test_cannot_create_payment_plan_without_down_payment_confirmation_for_bank_transfer()
    {
        // Create reservation with bank transfer but not confirmed
        $reservation = SalesReservation::factory()->create([
            'contract_id' => $this->offPlanContract->id,
            'contract_unit_id' => $this->unit->id,
            'marketing_employee_id' => $this->salesUser->id,
            ...
            'down_payment_status' => 'non_refundable',
            'down_payment_confirmed' => false, // Not confirmed
            'confirmed_at' => now(),
        ]);
        ...
        $response->assertStatus(422)
            ->assertJsonFragment([
                'message' => 'Payment plans can only be created after down payment is confirmed by Accounting (required for bank transfers/electronic payments)'
            ]);
    }

    public function test_can_create_payment_plan_with_confirmed_down_payment_for_bank_transfer()
    {
        // Create reservation with bank transfer and confirmed
        $reservation = SalesReservation::factory()->create([
            'contract_id' => $this->offPlanContract->id,
            'contract_unit_id' => $this->unit->id,
            'marketing_employee_id' => $this->salesUser->id,
            ...
            'payment_method' => 'bank_transfer',
            'down_payment_status' => 'non_refundable',
            'down_payment_confirmed' => true, // Confirmed
            'down_payment_confirmed_by' => $this->salesLeader->id,
    ...
    public function test_can_create_payment_plan_for_cash_without_confirmation()
    {
        // Create reservation with cash payment (doesn't require confirmation)
        $reservation = SalesReservation::factory()->create([
            'contract_id' => $this->offPlanContract->id,
            'contract_unit_id' => $this->unit->id,
            'marketing_employee_id' => $this->salesUser->id,
            ...
            'payment_method' => 'cash',
            'down_payment_status' => 'non_refundable',
            'down_payment_confirmed' => false, // Cash doesn't require confirmation
            'confirmed_at' => now(),
        ]);
    ...
    public function test_cannot_create_payment_plan_without_down_payment_confirmation_for_bank_financing()
    {
        // Create reservation with bank financing but not confirmed
        $reservation = SalesReservation::factory()->create([
            'contract_id' => $this->offPlanContract->id,
            'contract_unit_id' => $this->unit->id,
            'marketing_employee_id' => $this->salesUser->id,

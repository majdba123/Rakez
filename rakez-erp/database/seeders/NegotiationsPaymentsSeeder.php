...
class NegotiationsPaymentsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $pendingCount = min(70, $targetApprovals);
        $processedCount = max(0, $targetApprovals - $pendingCount);
        $approvalStatuses = array_merge(
            array_fill(0, $pendingCount, 'pending'),
            array_fill(0, (int) ceil($processedCount / 2), 'approved'),
            array_fill(0, (int) floor($processedCount / 2), 'rejected')
        );
        ...
        foreach ($approvalReservations as $index => $reservationId) {
            ...
            NegotiationApproval::firstOrCreate(
                ...
                [
                    ...
                    'approved_by' => $status === 'pending' ? null : Arr::random($leaderPool),
                    'status' => $status,
                    'negotiation_reason' => 'price',
                    'original_price' => $originalPrice,
                    'proposed_price' => $proposedPrice,
                    'manager_notes' => $status === 'rejected' ? 'Rejected by manager' : null,
                    'deadline_at' => now()->addHours(48),
                    'responded_at' => $status === 'pending' ? null : now()->subHours(6),
                ]
            );
        }

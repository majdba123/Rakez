...
class AccountingDepositService
{
    ...
    public function getPendingDeposits(array $filters = [])
    {
        ...
        return $query->orderBy('payment_date', 'desc')->paginate($filters['per_page'] ?? 15);
    }
    ...
    public function generateClaimFile(int $reservationId): array
    {
        ...
        $claimData = [
            ...
            'commission_percentage' => $reservation->commission->commission_percentage,
            'commission_amount' => $reservation->commission->net_amount,
            'commission_source' => $reservation->commission->commission_source,
            'deposit_amount' => $reservation->deposits->sum('amount'),

...
class CreditBookingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $bookings = $query->orderByRaw('COALESCE(confirmed_at, created_at) DESC')->paginate($perPage);
    ...
    public function cancelled(Request $request): JsonResponse
    {
        try {
            ...
            if ($request->has('from_date')) {
                $query->whereDate('cancelled_at', '>=', $request->input('from_date'));
            }
            if ($request->has('to_date')) {
                $query->whereDate('cancelled_at', '<=', $request->input('to_date'));
            }
            if ($request->has('contract_id')) {
                $query->where('contract_id', $request->input('contract_id'));
            }
            ...
            $perPage = min((int) $request->input('per_page', 15), 100);
            $bookings = $query->orderBy('cancelled_at', 'desc')->paginate($perPage);

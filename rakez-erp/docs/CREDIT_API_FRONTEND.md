...
| POST | `/api/credit/bookings/{id}/claim-file` | Generate claim file for booking |
| GET | `/api/credit/claim-files/{id}` | Claim file detail |
| POST | `/api/credit/claim-files/{id}/pdf` | Generate PDF |
| GET | `/api/credit/claim-files/{id}/pdf` | Download PDF |

**GET claim-files query (optional):** `per_page` (default 15, max 100).
